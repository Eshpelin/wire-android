...
import com.waz.zclient.views.{AvailabilityStatus, ConversationBadge}
...
class NormalConversationListRow(context: Context, attrs: AttributeSet, style: Int) extends FrameLayout(context, attrs, style)
  ...
  (for {
    ...
    title.setText(name)
    title.setCompoundDrawablesWithIntrinsicBounds(AvailabilityStatus.availabilityIds(av).iconId, 0, 0, 0)
  }
  ...
  def setConversation(conversationData: ConversationData): Unit = {
    if (this.conversationData.forall(_.id != conversationData.id)) {
      ...
      controller.availability(conversationData.id).head.foreach { av =>
        title.setCompoundDrawablesWithIntrinsicBounds(AvailabilityStatus.availabilityIds(av).iconId, 0, 0, 0)
      }(Threading.Ui)
